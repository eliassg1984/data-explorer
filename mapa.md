...
| `grupo_nav` | str | Entradas de `REPORTES` que comparten este valor se dibujan como **UN solo botón** en el rail (`navegacion.py::inject_navegacion`), navegando al último miembro visitado (session_state `_ultimo_<grupo>`). UN solo grupo hoy: Recetas/Nueva Receta (`"Recetas"`, ver reglas #97 y #303). Requerimientos/Salidas fue el otro hasta el 2026-09-05, cuando los dos se fusionaron en un reporte único y el grupo dejó de tener sentido — un grupo de un miembro es un botón normal con pasos de más (ver regla #322). Sigue habiendo DOS entradas reales en `REPORTES` por grupo (cada una con su `archivo`/cfg propios); el grupo es puramente de presentación. El ícono del botón agrupado sale del PRIMER miembro del grupo en el orden del dict — por eso conviene que los dos miembros compartan el mismo `icono` (evita que la elección dependa del orden). |
