...
| `estado_rango.py` | **Dueño único** del eje temporal de la franja superior: rango (`clave_rango`, `asegurar_rango`, `atajos_rango`, `aplicar_atajo`, `clave_eco`/`restaurar_eco` — el espejo que lo salva de la recolección de widgets, regla #458) **y corte** (`clave_corte`, `clave_modo`, `modo_fecha`, `corte_vigente`, `aplicar_corte`, `volver_a_rango`). Nadie escribe esas claves fuera de este módulo — ver reglas #24 y #62. Su hermano chico es `graficos/periodo.py`, el rango POR VISTA (regla #133): no le disputa nada — la franja sigue mandando y una vista que no lo importe se comporta igual que antes. |
...
| `tablas/` | **Paquete de tablas AgGrid** (refactor 2026-08-01; antes un `tablas.py` de 2.028 líneas). `__init__.py` re-exporta la API pública. `_css.py` (CSS de grid y paneles), `_config.py` (estilos de celda/fila, sidebar, totales), `desktop.py` (`renderizar_aggrid_desktop`), `movil.py` (`renderizar_aggrid_movil`), `compras.py` (`renderizar_aggrid_compras`), `ajuste_pivote.py` (`renderizar_aggrid_pivote_ajuste`, la tabla de Ajuste › Evolución —hasta el 2026-09-23 la vista suelta «Por fecha»—, ver reglas #25 y #501), `compras_volatilidad.py` (las dos grillas de Volatilidad y `_css_look`, el look de las tablas de compras), `compras_vs_ano_pasado.py` (la tabla de detalle de «Vs año pasado») y `compras_semanal.py` (2026-09-14: las dos grillas del detalle de Semanal — documentos del período | líneas del elegido —, regla #440) y `ajuste_familias.py` (2026-09-16: la tabla de familias de Ajuste › Cascada y sus dos desgloses, con el look y la fila marcada de Semanal pero key estable para no perder el orden, regla #441). `renderizar_tabla_compras` se borró el 2026-08-08 (llevaba desde 2026-08-01 sin llamadores). |
| `graficos/` | **Paquete de dashboards de gráficos** (refactor Fase 2, 2026-07-25). `__init__.py` es solo el dispatcher: dict `_DASHBOARDS = {reporte: render_fn}` (no cadena de if/elif), más `renderizar_graficos_reporte` (entry point) y `tiene_dashboard(reporte)` (para que `app.py` no enumere reportes ni importe `_DASHBOARDS`; ver regla #50). `render_vista_pills` (pestañas Gráficos/Tabla sueltas en la franja) se ELIMINÓ 2026-08-04: ver regla #18. Cada dashboard vive en su archivo: `base.py` (infraestructura compartida: cards nativos, motor genérico, resolución de columnas, helpers de layout), **`ajuste/` es un paquete** (refactor 2026-08-08; antes un `ajuste.py` de 2.607 líneas — el fichero con MÁS churn del repo, 80 de los últimos 200 commits): una vista por módulo — `_comun.py` (layout del rail, fechas de corte, periodos), `_evolucion.py` (la ÚNICA vista de la categoría Tiempo desde el 2026-09-23: serie de sobrante/faltante/neto + mini-gráficos por familia + la tabla pivote, con el mismo grano y el mismo rango; reemplaza a Evolución, Comparativa mensual y Por fecha de corte, regla #501), `_pivote.py` (la tabla de esa vista), `_cascada.py`, `_panel_analisis.py`, `_heatmap.py`, `_distribucion.py` — y `__init__.py` con la config del rail, `categoria_rango_ajuste` y el entry point. Ojo: la **Cascada ya no dibuja una cascada** (conserva el nombre, la URL y el prefijo `ajcas_`): desde el 2026-09-16 es una tabla AgGrid de familias —faltó, sobró, faltó + sobró, saldo, valorizado total (la escala del stock, regla #483), productos del 80 % y exactitud, cada cabecera con su fuente— y un detalle de la familia en foco con tres pestañas (por producto, por área, y por corte, que lista TODOS los cortes del año, del más nuevo al más viejo); la grilla vive en `tablas/ajuste_familias.py`. Ver regla #441, `ventas.py` (`ventas_resumen.py` aporta su vista "Resumen ejecutivo" — KPIs + venta diaria coloreada por tendencia + ticket promedio + top platos; nació con un candlestick, ver por qué se dio de baja en la regla #85) y `ventas_comparativo.py` (vista "Año Pasado": barras agrupadas Actual vs Año Pasado en día/semana/mes, con toggle de alineación fecha-calendario / día-de-semana en día, feriados y findes marcados, recorte del período en curso, modo "Descomposición" (%Δ venta/pax/ticket en un solo eje) y drill por clic al ranking de platos del período — ver reglas #86, #87 y #88), **`ventas_horario.py`** (2026-08-14, vista "Por hora": mapa de calor día × hora de hasta 4 períodos comparados en franjas, marcas rectangulares por arrastre —una por panel tocado—, drill con medidas a elección y árbol Grupo › Sub Grupo › Plato › **Tipo de descuento**; reusa los helpers de calendario de `ventas_comparativo` en vez de duplicarlos y sólo agrega la granularidad Año — ver reglas #112 a #115. Abre SIEMPRE en el período EN CURSO (uno solo: comparar es explícito y tiene su botón), recortado al último día con datos; los períodos a comparar NO tienen que ser consecutivos ni recientes — la lista es el atajo y el `date_input` abre el calendario entero. El eje de horas va en am/pm, y en el eje de días los fines de semana van en negro y los feriados en ámbar, con el MISMO calendario `_feriados_peru` que pinta las bandas de la vista Año Pasado), `inventario.py` (v3: Por área / Por familia / Productos / Tabla apiladas; las dos de ranking son tres TABLAS clickeables en cadena —ranking → desglose del nivel siguiente → todos los productos del recorte— desde el 2026-09-13, ver regla #403) y **`inventario_productos.py`** (2026-09-17, la sección «Productos», que reemplaza a «Buscar producto»: el listado ENTERO de productos en una AgGrid —Código · Familia · Subfamilia · Nombre · Unidad kardex · Precio unitario · Cantidad · Valorizado total— con las áreas donde cada uno tiene stock desplegables por clic en su fila, y filtros en la fila del título: Área (abre en Almacén central, Cocina, Barra, Producción y Salón), Familia y Subfamilia —las tres de selección múltiple, en disparadores compactos que abren un panel— más el buscador. El valor de esos tres NO vive en la key de su widget: el popover no monta su contenido cerrado y se perdía lo que Python les escribía, ver regla #467. El despliegue es de Community, sin master/detail: filas planas de dos tipos, filtro externo y `postSortRows`, todo en el navegador (`update_on=[]`); ver regla #466), **`movimientos.py` es UN solo dashboard desde el 2026-09-05**, con OCHO secciones apiladas: dos que miran los DOS parquets —Evolución (requerido vs dado de baja en barras agrupadas) y Proporción dada de baja (qué parte de lo requerido terminó dándose de baja, por producto: cociente y no resta, ver regla #323)— más tres de REQUERIMIENTOS (Sub Almacén, Top productos, Tabla pivote) y tres de SALIDAS (Tipo de descargo, Top productos, Tabla). Hasta esa fecha eran DOS reportes hermanos que un chip Requerimiento/Salidas alternaba; se fusionaron a pedido el día después de que la Evolución empezara a mostrar los dos lados juntos («esto ya no debería estar, ya que ahora muestra ambos»), con el mismo movimiento que Recetas el 2026-09-04. Ver regla #322. En la fusión se cayeron las dos vistas de Salidas que colgaban de `Sub Almacen` —una columna que `salidas.parquet` NO trae—: estaban muertas desde que nacieron, medido en la regla #98. `df_f` es el de REQUERIMIENTOS (el reporte apunta a requerimientos.parquet) y salidas.parquet lo carga el módulo con `data.cargar`, mismo patrón que `recetas.py`. Ojo con las dos Tablas: la de requerimientos va por `tabla_cb` (la pivote de `app.py::_cb_requerimientos_tabla`) y la de salidas llama a `renderizar_aggrid_desktop` directo — `tabla_cb` recorta con el `cols_mostrar` del reporte activo y reventaría con las columnas del otro parquet. Las dos le pasan al grid el nombre LITERAL de su lado, que es de lo que `tablas/desktop.py` cuelga el modo pivote y la paginación. **`movimientos_comun.py`** guarda las piezas que miran los dos parquets a la vez: la Evolución fusionada, el Comparativo (Pedido vs Baja) y `_rango_vigente()`, el recorte por fecha que los tres sitios comparten — ver reglas #98, #320 y #321. `legacy.py` (Inventario v1) se borró el 2026-08-08: 421 líneas sin un solo import. **`compras/` es a su vez un paquete** (refactor 2026-08-01; antes un `compras.py` de 2.835 líneas): un drill por archivo — `_comun.py` (helpers, incluye `_periodo_serie` para granularidad temporal — reusar desde ahí, no duplicar), `proveedor.py`, `producto.py` (2026-08-17, reemplaza a "Precio top 10" + "Precio por compra" + `cantidad.py`/"Cantidad por producto", que se borraron ese día: ranking de TODOS los productos —valor, cantidad, UM, precio real de inicio/fin de período y su variación— con el mismo patrón tabla-ranking + clic-para-enfocar que `proveedor.py`; el producto en foco muestra su evolución con un selector de texto plano Precio/Cantidad/Valor × Semana/Mes/Año que fusiona el promedio del período con el precio real de cada compra en un solo gráfico; encima del ranking, en su MISMA tarjeta, dos paneles Familia | Subfamilia lo recortan al clic (desde el 2026-09-12; antes eran tres paneles en una tarjeta aparte, el tercero con los productos del grupo). Desde ese mismo día la fila va espejada —evolución a la IZQUIERDA con ventana propia que abre en 3 meses, tablas a la derecha con la fecha de la sección en su fila de títulos—, sin filtro de proveedores y sin techo de alto en sus tarjetas — ver reglas #128, #374, #381 y #382), `volatilidad.py` (`evolucion.py` —la vista "Evolución prov." del rail— se borró el 2026-08-16 a pedido: 377 líneas que quedaron sin un solo import al sacar la dimensión del rail; su pregunta la responde ahora el gráfico de evolución que vive DENTRO del drill de Proveedor) (ranking de insumos por volatilidad de precio → candlestick semanal → compras de la semana clickeada; ver regla #74. Desde el 2026-09-12 el ranking ocupa el ancho entero de la tarjeta y el drill va debajo, candlestick y semana mitad y mitad — regla #384; la grilla se desliza hacia atrás por toda la ventana y la columna «Volatilidad» es opcional y lleva escrito su período — regla #385; el título y los controles van en un panel a la derecha de la tabla — regla #386), `semanal.py` (en el rail, «Compras por período»: se llamaba «Semanal» y es la PRIMERA sección de Compras desde el 2026-09-19 —el nombre interno, `compras_sec_semanal` y las keys `compras_sem_*`, quedó—. 2026-09-04: una barra por período —Día/Semana/Mes/Año o «Por documento», que agrupa por COMPRA y no por fecha— con el detalle del período que se clickea. Desde el 2026-09-19 filtra por Familia › Subfamilia › Proveedor › Producto propios de la tarjeta, y cada barra de período va partida en tres tramos con el total, los documentos y la variación contra la barra anterior —«parcial» donde el rango corta el período, regla #470. Vivía inline en `__init__.py` hasta que su tarjeta sumó el selector de fecha de `_comun.py::selector_fecha_tarjeta`: escalar a `st.rerun(scope="app")` desde el fragment de la SECCIÓN deja a esa misma sección sin repintar, así que el drill necesita `@st.fragment` propio a nivel de módulo — ver regla #311) — y `__init__.py` con la config del rail y `renderizar_graficos_compras`. `_COMPRAS_RAIL_CATEGORIAS` movió "Producto" a la categoría Dimensión (junto a Proveedor): al cubrir precio+cantidad+valor a la vez ya no es una vista de "Precios". El drill "Familia" (Familia→Subfamilia→productos) se eliminó el mismo día por redundante con el ranking por Familia que ya trae Producto — ver regla #129. El drill de Proveedor se siguió partiendo el 2026-08-08 (era una función de 1.577 líneas): `_css_proveedor.py` (sus 527 líneas de CSS, que NO van a `estilos/` a propósito — ver su docstring), `_etiquetas_proveedor.py` (texto de las barras: `fmt_k`, `abrev_nombre`, `etiqueta_serie`, `sufijo_granularidad`; puras y con asserts de valor en `test_graficos.py`) y `_documentos_proveedor.py` (`tabla_documentos`, la AgGrid pivote del pie). **`_calendario.py`** existió el 2026-08-24 y se eliminó ese mismo día: era el selector de dos meses que la vista "Semanal" dibujaba dentro de su tarjeta. Al apilarse las vistas de Compras en una sola página, las seis pasan a dibujarse en la MISMA corrida y dos dueños de la clave del rango dejaron de poder convivir — Compras reventaba en cada carga. El rango volvió a ser del reporte (la franja de arriba). La lección de construcción sigue siendo válida y vive en la regla #203; el porqué de la marcha atrás, en la #210. `documentos_sunat.py` (2026-08-19, drill "Documentos SUNAT": los comprobantes que los proveedores emitieron hacia el RUC, vía `sunat.py` — el único drill de Compras cuyo dato no sale del parquet, y el único que NO respeta los chips Familia/Subfamilia; ver regla #139) es el drill más nuevo, y el único con una vista "Cruce" que SÍ vuelve a tocar el parquet: `cruzar_con_parquet` compara cada comprobante del SIRE contra `compras.parquet` por documento, acotando por fecha antes de armar la clave para no colisionar entre proveedores que reusan la misma serie — ver regla #143. Cuando un dashboard crezca así, partirlo del mismo modo. **Agregar un dashboard nuevo = crear `graficos/<nombre>.py` + 1 línea en `_DASHBOARDS`.** |
| `estilos/` | **Paquete del CSS global** (refactor 2026-08-01; antes un `estilos.py` de 1.700 líneas). `__init__.py` mantiene la API pública (`TAM_FUENTE`, `get_css`, `inject_css`) y concatena las secciones. Una sección por módulo, con prefijo numérico que marca el orden: `_00_base`, `_20_compras_rail`, `_26_rails_scroll`, `_27_pila`, `_28_arbol` (desde 901px: el rail en árbol y la franja de contexto, regla #472), `_30_filtros`, `_40_ajuste_franja`, `_50_fecha`, `_60_calendario`, `_70_chrome`, `_80_cards`, `_90_franja_inferior`, `_99_movil`. (`_10_vista` existió hasta el 2026-08-08: estilaba el selector Gráficos/Tabla y quedó 100% huérfano al borrarse ese widget — ver regla #49.) **El orden de `_SECCIONES` es parte del comportamiento**: hay `!important` en ambos lados de varios conflictos, así que gana la regla que va DESPUÉS — por eso `_99_movil` cierra. |
| `navegacion.py` | **Desde 901px (2026-09-19, regla #472): los reportes del rail en árbol** —las vistas del activo las anida `graficos/base.py::_render_rail`; la geometría del árbol, plegado/asomado/fijado, vive en `estilos/_28_arbol.py`— **y el contenido de la franja superior de contexto** (`_html_barra_contexto`: reporte, vista en pantalla, KPIs). Lo que sigue describe el esquema de 769 a 900px y la historia: **Rail vertical izquierdo de Reportes** (ícono + label, key `compras_tabs_row`; los KPIs por reporte —los montos `S/`— se quitaron del rail el 2026-09-22 a pedido, siguen en la franja de contexto vía `_html_barra_contexto`) + el CSS de la franja horizontal donde `graficos/base.py::_render_rail` dibuja Vistas (key `nav_rail`, contenedor por POSICIÓN — ver regla #170; en escritorio ≥901px ya no se dibuja desde el 2026-09-14, y la fecha y Filtros que vivían en ella subieron a la franja de reportes — regla #419) + el CSS de la cabecera fija (`_CSS_AJUSTE`, se inyecta en TODOS los reportes). Botón de refresco y pestillo al pie del rail, cada uno en su propio `@st.fragment`. En móvil el rail fluye como tira horizontal de chips (900px) y la franja de Vistas baja a bottom-nav (768px). Fue franja horizontal solo-texto de 2026-08-18 a 2026-08-22 (regla #132) — la #170 revirtió a rail vertical y sumó íconos/KPIs. |
| `inyecciones/` | **Paquete de JS/HTML inyectado** (refactor 2026-08-01; antes un `inyecciones.py` de 1.813 líneas). `_iframe.py` (`inyectar_html`, el primitivo que usan TODAS: el `<script>` va en un iframe con `srcdoc`; antes era `components.html`, que Streamlit retiró — regla #204), `_fragmentos.py` (CSS/JS compartido), `grid.py` (salud, altura, maximizar, panel de columnas), `paginacion.py`, `inspector.py` (herramienta de desarrollo), `diseno.py` (modo de diseño visual, `?debug=1&diseno=1` — lee el pin de `inspector.py`, ver regla #46), `varios.py` (overlay de errores, fullscreen, footer, calendario). Los dos blobs de JS grandes viven aparte desde el 2026-08-08: `_inspector_js.py` (1.381 líneas) y `_diseno_js.py` (794). Sus funciones quedaron en 34 y 5 líneas. **Si tocas esos módulos, lee antes la regla #56** — extraerlos rompió el inspector de una forma que ni `ruff` ni los tests pueden ver. Ninguna función depende de otra (la excepción de solo-lectura de `diseno.py` está documentada en la regla #46): las únicas dependencias internas apuntan a las constantes de `_fragmentos.py` y a `inyectar_html` de `_iframe.py`. |
...
| `label_largo` | str | Nombre de MOSTRAR en la franja de contexto y la cabecera del rail (anchas, ≥769px). Sin él cae en la CLAVE del dict — que es lo que ve el resto de los reportes. Existe para renombrar lo que ve el usuario sin tocar la clave interna (dispatcher, asistente, slug CSS, citas de `arquitectura.md`): hoy lo declaran "Inventario Valorizado" → **Stock e Inventario**, "Movimientos" → **Movimientos** (antes "Movimientos de Almacén") y "Recetas" → **Recetas y Costos** (2026-09-22; hasta ese día el rail de Recetas era además un botón AGRUPADO junto a "Nueva Receta", que llevaba el mismo rótulo en `grupo_nav` — «Nueva receta» bajó a ser una VISTA de Recetas ese mismo día, ver `graficos/recetas.py::_RAIL_CATEGORIAS`). Las vistas de ese reporte también se rebautizaron a «Stock por Producto/Área/Familia» cambiando SÓLO el rótulo de sus tuplas del rail, no el id interno (`graficos/inventario.py::_INVENTARIO_RAIL_CATEGORIAS`). |
...
| `archivos_extra` | tuple | Parquets ADICIONALES que el dashboard dibuja además de `archivo`. `app.py` sigue cargando uno solo y pasándolo como `df_f`; el resto los carga el dashboard con `data.cargar`. Existe para que el botón **Refrescar** los pida TODOS (`navegacion.py::boton_refresco`) y para que `app.py` monte un `_vigilar_refresco` por cada uno — sin esto, la mitad secundaria de la página no tendría forma de actualizarse desde la UI. Hoy solo Recetas (`recetabase.parquet`). |
...
| `grupo_nav` | str | Entradas de `REPORTES` que comparten este valor se dibujan como **UN solo botón** en el rail (`navegacion.py::inject_navegacion`), navegando al último miembro visitado (session_state `_ultimo_<grupo>`). **Hoy no queda ninguno vigente** — el mecanismo sigue soportado, pero desde el 2026-09-22 no hay un solo grupo real en `REPORTES`. Hubo dos: Requerimientos/Salidas hasta el 2026-09-05 (los dos parquets se fusionaron en un reporte único, regla #322) y Recetas/Nueva Receta hasta el 2026-09-22 (reglas #97 y #303; "Nueva receta" bajó a ser una VISTA del reporte Recetas, un grupo de un solo miembro es un botón normal con pasos de más). Sigue habiendo DOS entradas reales en `REPORTES` por grupo (cada una con su `archivo`/cfg propios); el grupo es puramente de presentación. El ícono del botón agrupado sale del PRIMER miembro del grupo en el orden del dict — por eso conviene que los dos miembros compartan el mismo `icono` (evita que la elección dependa del orden). |
