...
# vueltas de pedido), pasó a `graficos/compras/_comun.py` el 2026-08-26
# cuando se pidió "el mismo selector" para el Ranking de Productos, y llegó
# ACÁ el 2026-09-05 con el tercer pedido igual — la Evolución de Movimientos,
# que ya no es de Compras. Copiarlo hubiera sido duplicar ~190 líneas y,
# peor, N sitios donde arreglar el próximo detalle.
#
# Vive pegado a `selector_escala` porque ésa es la pieza que abre adentro:
# el panel del trigger son los cuatro atajos relativos MÁS ese riel.
#
# OJO — SU CSS NO ESTÁ EN `estilos/`. Cada consumidor inyecta el suyo con
# `st.markdown` al dibujar, acotado a SU prefijo de key: los cuatro de
# Compras van en `graficos/compras/_css_proveedor.py` y el de Movimientos
# en `graficos/movimientos_comun.py`. No es prolijidad perdida — es la
# posición en la cascada: los valores están afinados contra un `<style>`
# que llega DESPUÉS de `estilos/`, y mudarlos allá los adelantaría. Ver
# arquitectura.md regla #320.
#
...
def _render_rail(categorias, state_key, btn_prefix="graf_btn_",
    ...
    if secciones:
        ...
        # la franja de vistas se desliza y el rail sube a tocar la de
        # reportes, así que la banda donde vivía la cabecera ya no existe en
        # ese estado. Quién es el reporte lo dice la franja de KPIs que entra
        # en ese mismo gesto.
...
# NOMBRE PROPIO: se mudo a `graficos/compras/_etiquetas_proveedor.py`.
#
# Aca vivio una segunda `nombre_propio` hasta el 2026-09-11, con el mismo
# nombre y el mismo proposito que la de ese modulo, y diferian en 48 de los
# 773 proveedores del parquet: el mismo proveedor salia escrito distinto
# segun lo formateara Volatilidad (importaba esta) o el drill de Proveedor
# (importa la otra). Quedo una sola, con las reglas de las dos, y vive alla
# porque `base.py` no puede importar de `graficos/compras/` -- el paquete
# importa de `base.py` al cargarse, asi que es un ciclo. Volatilidad ahora
# la importa de su modulo. Ver arquitectura.md regla #379.
