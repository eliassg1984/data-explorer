...
def _ventas_resumen(d, col_venta, col_fecha, col_pax, col_pedido, col_prod, col_cant):
    ...
    k1.metric("💰 Ventas totales", f"S/ {total_venta:,.0f}")
    k2.metric(f"👥 {vol_label}" if vol_label else "👥 Clientes",
              f"{total_pax:,.0f}" if vol_label else "—")
    k3.metric("🎟️ Ticket promedio",
              f"S/ {ticket_prom:,.2f}" if ticket_prom else "—")
    k4.metric("🏆 Mejor día", f"S/ {mejor_valor:,.0f}",
              delta=f"{mejor_dia:%d/%m}", delta_color="off")
    k5.metric("📈 Días en alza", f"{alzas}/{len(g) - 1}",
