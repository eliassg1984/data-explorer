...
from graficos.base import (
    ...
    _resolver, esqueleto_pila, publicar_contexto_ia,
...
#
# Fuera de la pila y por qué: `Personalizado` es un editor con estado propio;
# `Documentos SUNAT` se lleva prestado el único selector de fecha de la app.
_PILA = (
...
def renderizar_graficos_compras(df_f, nombre_reporte, df_full=None, tabla_cb=None):
    ...
    # Dos excepciones a la pila de abajo, cada una por su motivo:
    #
    #   · Personalizado es un EDITOR con estado propio (el constructor de
    #     gráficos), no una lectura corrida.
    #   · Documentos SUNAT se lleva prestado el ÚNICO selector de fecha de
    #     la app (`franja_fecha.render()`, la misma función que llama
    #     `app.py`): si estuviera siempre en pantalla, las demás vistas se
    #     quedarían sin control de fecha arriba. Entra a la pila cuando
    #     tenga rango propio — ver `vista_quiere_fecha_propia`.
    ...
    #
    # OJO con las keys de tarjeta: Volatilidad, Semanal y Personalizado
    # usaban las tres `ajuste_graf_card_izq_compras`. No chocaban porque
    # nunca coexistían; apiladas serían dos widgets con la misma key, que en
    # Streamlit es una excepción. Cada una lleva ahora su sufijo, y el
    # prefijo `ajuste_graf_card_` se conserva porque de él cuelga el CSS de
    # tarjeta (el clamp de una pantalla en `estilos/_80_cards.py`).

    # ── Fase 1: el ESQUELETO, entero, antes de construir nada ────────────
    # Los seis contenedores de sección nacen ya, con su key y un hueco
    # dentro. Dos cosas que esto arregla:
    #
    #   · la página deja de CRECER bajo el cursor. Antes cada sección
    #     aterrizaba y corría lo que estabas leyendo; ahora la altura está
    #     reservada desde el principio (ver `estilos/_27_pila.py`).
    #   · el scrollspy deja de depender de la suerte. Su observador se monta
    #     en `_render_rail`, que corre ANTES que todo esto: si las secciones
    #     no existieran todavía, `querySelector` no encontraría nada y el
    #     rail no marcaría nunca. Funcionaba porque el iframe del componente
    #     carga tarde — o sea por casualidad, no por diseño.
    #
    # `st.empty()` porque es el único contenedor que se puede REEMPLAZAR:
    # el `.container()` de la fase 2 pisa el hueco en su sitio.
    _hueco = {}
    for _clave, _vista in _PILA:
        with st.container(key=_clave):
            _hueco[_clave] = st.empty()
            _hueco[_clave].markdown(esqueleto_pila(_vista),
                                    unsafe_allow_html=True)

    # ── Fase 2: el contenido, de arriba abajo ────────────────────────────
    # El orden importa: Streamlit manda cada elemento apenas lo construye,
    # así que este orden ES el orden en que el usuario los ve aparecer. Y
    # el usuario empieza por arriba.
    with _hueco["compras_sec_proveedor"].container():
    ...
    with _hueco["compras_sec_producto"].container():
    ...
    with _hueco["compras_sec_semanal"].container():
    ...
    with _hueco["compras_sec_tabla"].container():
